...
export class HotelsController {
  ...
  static async createHotel(req, res, next) {
    try {
      const newHotel = await HotelModel.createHotel(req.body);
  ...
  static async updateHotel(req, res, next) {
    try {
      const updatedHotel = await HotelModel.updateHotel(req.params.id, req.body);
      if (!updatedHotel) {
        return next(ENTITY_NOT_FOUND('Hotel'));
      }
  ...
  static async deleteHotel(req, res, next) {
    try {
      const deleted = await HotelModel.deleteHotel(req.params.id);
      if (!deleted) {
        return next(ENTITY_NOT_FOUND('Hotel'));
      }
